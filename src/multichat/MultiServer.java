...
public class MultiServer {
	...
	// 생성자
	public MultiServer() {
		// 클라이언트의 이름과 출력스트림을 저장할 HashMap컬렉션생성
		clientMap = new HashMap<String, PrintWriter>();
		// HashMap 동기화설정. 쓰레드가 사용자정보에 동시에 접근하는 것을 차단
	...
	public void init() {
		try {
			...
			while(true) {
				...
				/*
				클라이언트의 메세지를 모든 클라이언트에게 전달하기 위한 쓰레드 생성 및 시작.
				한 명당 하나씩의 쓰레드가 생성된다.
				 */
	...
	// 접속된 모든 클라이언트들에게 서버의 메세지를 전달하는 메소드
	...
	class MultiServerT extends Thread{
		...
		public void run() {
			...
			try {
				...
				// 방금 접속한 클라이언트르 ㄹ제외한 나머지에게 사용자의 입장을 알려준다.
				sendAllMsg("", name + "님이 입장하셨습니다.", "All");
				if(clientMap.containsKey(name))	// 존재함
				{
				...
				while(in!=null) {
					 ...
					 */
					if(inMsg.charAt(0)=='/') {
						// 만약 /로 시작한다면 귓속말 등 명령어이다.
						/*
						귓속말은 아래와 같이 전송하게 된다.
						-> /to 사용자이름 대화내용
							: 따라서 split()을 통해 space로 문다열을 분리한
						 */
						String[] strArr = inMsg.split(" ");
						
						if(strArr[0].equals("/to")) {
							// 함수 호출시 One이면 한명에게만 메세지 전송
//							/to bbb bbb에게만 보이는 메세지입니다.
//							0    1   2       3     4
							String msgContent = "";
							...
							sendAllMsg(name, inMsg, "All");	// All 이면 전체전송
					}
					else {
						// 만약 /로 시작하지 않으면 일반 메세지이다.
						sendAllMsg(name, inMsg, "All");
					}
				}
			}
			catch(Exception e) {
